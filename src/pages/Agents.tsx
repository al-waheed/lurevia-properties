      ...
      <section className="border-b border-aurel-border bg-white">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10 lg:py-20">
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-aurel-muted">
            Meet the property specialists behind AUREL. Experienced, thoughtful
            ...
            {agents.map((agent) => (
              <article key={agent.id} className="group">
                <Link to={`/agents/${agent.id}`}>
                  <div className="aspect-[4/5] overflow-hidden bg-aurel-border">
                    <img
                      src={agent.image}
                      alt={agent.name}
                  ...
                  <div className="pt-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h2 className="font-display text-2xl font-semibold text-aurel-text">
                          {agent.name}
                        </h2>
                      ...
                      <ArrowUpRight
                        size={19}
                        className="mt-1 text-aurel-muted transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
                      />
                    </div>

                    <div className="mt-4 flex items-center gap-1.5 text-xs text-aurel-muted">
                      <MapPin size={14} />
                      {agent.location}
                    </div>
                  </div>
